...
import { utils, Client } from '@toruslabs/tss-client';
...
class Web3AuthMPCCoreKit {
  ...
  async init() {
    ...
    if (params.handleRedirectResult && ((_window = window) !== null && _window !== void 0 && _window.location.hash.includes("#state") || (_window2 = window) !== null && _window2 !== void 0 && _window2.location.hash.includes("#access_token"))) {
  ...
  async setupProvider() {
    ...
    const sign = async msgHash => {
      ...
      const [sockets] = await Promise.all([utils.setupSockets(tssWSEndpoints, randomSessionNonce), tss.default(tssImportUrl)]);
      const participatingServerDKGIndexes = nodeIndexes;
      const dklsCoeff = utils.getDKLSCoeff(true, participatingServerDKGIndexes, tssShareIndex);
      ...
      for (let i = 0; i < participatingServerDKGIndexes.length; i++) {
        ...
        serverCoeffs[serverIndex] = utils.getDKLSCoeff(false, participatingServerDKGIndexes, tssShareIndex, serverIndex).toString("hex");
      }
...
export { BrowserStorage, COREKIT_STATUS, CURVE, DEFAULT_CHAIN_CONFIG, DELIMITERS, ERRORS, FIELD_ELEMENT_HEX_LEN, FactorKeyTypeShareDescription, MAX_FACTORS, Point, SCALAR_LEN, SOCIAL_FACTOR_INDEX, SOCIAL_TKEY_INDEX, TssSecurityQuestion, TssSecurityQuestionStore, TssShareType, USER_PATH, VALID_SHARE_INDICES, WEB3AUTH_NETWORK, Web3AuthMPCCoreKit, addFactorAndRefresh, deleteFactorAndRefresh, generateFactorKey, generateTSSEndpoints, getHashedPrivateKey, getWebBrowserFactor, keyToMnemonic, mnemonicToKey, parseToken, scalarBNToBufferSEC1, storageAvailable, storeWebBrowserFactor };
