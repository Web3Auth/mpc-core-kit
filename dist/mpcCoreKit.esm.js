...
class AsyncStorage {
  ...
  static getInstance(key, storageKey) {
    if (!this.instance) {
...
// eslint-disable-next-line no-use-before-define
_defineProperty(AsyncStorage, "instance", void 0);
async function asyncStoreFactor(factorKey, mpcCoreKit, storageKey) {
  ...
  await currentStorage.set(tkeyPubX, JSON.stringify({
    factorKey: factorKey.toString("hex").padStart(64, "0")
  }));
}
async function asyncGetFactor(mpcCoreKit, storageKey) {
  const metadata = mpcCoreKit.tKey.getMetadata();
  const currentStorage = AsyncStorage.getInstance("mpc_corekit_store", storageKey);
  const tkeyPubX = metadata.pubKey.x.toString(16, FIELD_ELEMENT_HEX_LEN);
  const tKeyLocalStoreString = await currentStorage.get(tkeyPubX);
...
export { AsyncStorage, BrowserStorage, COREKIT_STATUS, CURVE, DEFAULT_CHAIN_CONFIG, DELIMITERS, ERRORS, FIELD_ELEMENT_HEX_LEN, FactorKeyTypeShareDescription, MAX_FACTORS, MemoryStorage, Point, SCALAR_LEN, SOCIAL_FACTOR_INDEX, SOCIAL_TKEY_INDEX, TssSecurityQuestion, TssSecurityQuestionStore, TssShareType, USER_PATH, VALID_SHARE_INDICES, WEB3AUTH_NETWORK, Web3AuthMPCCoreKit, addFactorAndRefresh, asyncGetFactor, asyncStoreFactor, deleteFactorAndRefresh, generateFactorKey, generateTSSEndpoints, getHashedPrivateKey, getWebBrowserFactor, keyToMnemonic, mnemonicToKey, parseToken, scalarBNToBufferSEC1, storageAvailable, storeWebBrowserFactor };
