...
__webpack_require__.d(__webpack_exports__, {
  AuthenticatorService: () => (/* reexport */ AuthenticatorService),
  ...
  Web3AuthStateFromJSON: () => (/* reexport */ Web3AuthStateFromJSON),
  ...
  refreshTssShares: () => (/* reexport */ refreshTssShares),
  remoteRefreshTssShares: () => (/* reexport */ remoteRefreshTssShares),
...
async function remoteRefreshTssShares(tKey, factorPubs, tssIndices, signatures, remoteClient) {
  ...
  let finalSelectedServers = (0,common_types_namespaceObject.randomSelection)(new Array(rssNodeDetails.serverEndpoints.length).fill(null).map((_, i) => i + 1), Math.ceil(rssNodeDetails.serverEndpoints.length / 2));
  const verifierNameVerifierId = tKey.serviceProvider.getVerifierNameVerifierId();
  ...
  const factorEnc = tKey.getFactorEncs(common_types_namespaceObject.Point.fromCompressedPub(remoteClient.remoteFactorPub));
  const dataRequired = {
    factorEnc,
  ...
  tKey.metadata.addTSSData({
    ...
    tssPolyCommits: result.tssPolyCommits.map(commit => common_types_namespaceObject.Point.fromJSON(commit)),
    factorPubs: result.factorPubs.map(pub => common_types_namespaceObject.Point.fromJSON(pub)),
...
;// CONCATENATED MODULE: external "hi-base32"
const external_hi_base32_namespaceObject = require("hi-base32");
var external_hi_base32_default = /*#__PURE__*/__webpack_require__.n(external_hi_base32_namespaceObject);
;// CONCATENATED MODULE: ./src/helper/authenticator/authenticatorService.ts
...
class AuthenticatorService {
  constructor(params) {
    defineProperty_default()(this, "backendUrl", void 0);
    defineProperty_default()(this, "coreKitInstance", void 0);
    defineProperty_default()(this, "authenticatorType", "authenticator");
    defineProperty_default()(this, "factorPub", "");
    defineProperty_default()(this, "tssIndex", void 0);
    const {
      backendUrl
    } = params;
  ...
  getDescriptionsAndUpdate() {
    ...
    base_namespaceObject.log.info("shareDescriptionsMobile", shareDescriptionsMobile);
  ...
  generateSecretKey() {
    const key = (0,eccrypto_namespaceObject.generatePrivate)().subarray(0, 20);
    return external_hi_base32_default().encode(key).toString().replace(/=/g, "");
  }
  async register(privKey, secretKey) {
    ...
    const sig = CURVE.sign((0,metadata_helpers_namespaceObject.keccak256)(Buffer.from(secretKey, "utf8")), Buffer.from(privKey.toString(16, 64), "hex"));
    const data = {
      pubKey: {
      ...
      sig: {
        ...
        v: new (external_bn_js_default())(sig.recoveryParam).toString(16, 2)
      },
      secretKey
    };
  ...
  async verifyRecovery(address, code) {
    ...
    return data ? new (external_bn_js_default())(data.factorKey, "hex") : undefined;
  }
...
;// CONCATENATED MODULE: ./src/helper/authenticator/smsService.ts
...
class SmsService {
  constructor(params) {
    defineProperty_default()(this, "backendUrl", void 0);
    defineProperty_default()(this, "coreKitInstance", void 0);
    defineProperty_default()(this, "authenticatorType", "sms");
    defineProperty_default()(this, "factorPub", "");
    defineProperty_default()(this, "tssIndex", void 0);
    const {
      backendUrl
...
class Web3AuthMPCCoreKit {
  ...
  async _UNSAFE_recoverTssKey(factorKey) {
    ...
    this.tkey.privKey = new (external_bn_js_default())(factorKey[1], "hex");
    const tssShares = [];
    const tssIndexes = [];
    const tssIndexesBN = [];
    ...
    const finalKey = (0,core_namespaceObject.lagrangeInterpolation)(tssShares, tssIndexesBN);
    await this.init();
  ...
  async deleteFactor(factorPub, factorKey) {
    ...
    if (this.state.remoteClient) {
      ...
      if (fpp.equals(Point.fromTkeyPoint((0,common_types_namespaceObject.getPubKeyPoint)(new (external_bn_js_default())(remoteStateFpp, "hex"))))) {
        throw new Error("Cannot delete current active factor");
      }
      await deleteFactorAndRefresh(this.tKey, factorPub, new (external_bn_js_default())(0),
  ...
  async setupRemoteClient(params) {
    ...
    const sharestore = common_types_namespaceObject.ShareStore.fromJSON(JSON.parse(metadataShare));
  ...
  async setupTkey(importTssKey) {
    if (this.state.remoteClient) {
      base_namespaceObject.log.warn("remote client is present, setupTkey are skipped");
      return;
    }
  ...
  async rehydrateSession() {
    try {
      ...
      if (result.factorKey) {
        ...
        metadataShare = common_types_namespaceObject.ShareStore.fromJSON(JSON.parse(result.remoteClient.metadataShare));
      }
  ...
  async createSession() {
    if (this.options.sessionTime === 0) {
      base_namespaceObject.log.info("sessionTime is 0, not creating session");
      return;
    }
    try {
  ...
  async copyOrCreateShare(newFactorTSSIndex, newFactorPub) {
    ...
    let userEnc;
    if (this.state.remoteClient) {
      const remoteFactorPub = common_types_namespaceObject.Point.fromCompressedPub(this.state.remoteClient.remoteFactorPub);
  ...
  async getSigningSignatures(data) {
    ...
    const sigPromise = this.options.authorizationUrl.map(async url => {
      const {
        sig
      } = await (0,http_helpers_namespaceObject.post)(url, {
        signatures: this.signatures,
  ...
  async remoteSign(msgHash) {
    ...
    const factor = common_types_namespaceObject.Point.fromCompressedPub(this.state.remoteClient.remoteFactorPub);
